...
void CPU::SearchM4ASoundDriverMain() {
  // TODO: support SoundMainRAM()!!!! how important is it really?
  // TODO: better signature, maybe?
  ...
  soundmain_address = 0xFFFFFFFF;
  soundinfo_ptr_address = 0x00000000;

  for (std::uint32_t address = 0x08000000; address <= 0x09FFFFFF; address++) {
    bool mismatch = false;
    for (std::uint32_t i = 0; i < sizeof(pattern); i++) {
      // TODO: do not generate bus cycles, idiot.
      if (ReadByte(address + i, Access::Sequential) != pattern[i]) {
        mismatch = true;
        break;
      }
    }
    if (!mismatch) {
      // TODO: do not generate bus cycles, you dummy.
      soundmain_address = address;
      soundinfo_ptr_address = ReadWord(soundmain_address + 0x6C, Access::Sequential);

      LOG_DEBUG("Found M4ASoundMain() at 0x{0:08X}, info_ptr = 0x{1:08X}", address, soundinfo_ptr_address);
      break;
    }
  }
}
